...
class cnnModel4(tNN.Module):
    def __init__(self, inputChannel, inputSize, numClassess):
        ...
        self.fc2 = tNN.Linear(128, numClassess)
...
def main():
    ...
    model = cnnModel4(inputChannel, inputSize, numClassess).to(device)
    ...
    torch.save(model, f'model5_CNN4_{datasetName}.pt')
    modelScripted = torch.jit.script(model)
    modelScripted.save(f'model5_CNN4_Scripted_{datasetName}.pt')
