...
def main():
    ...
    model = cnnModel5(inputChannel, inputSize, numClassess).to(device)
    ...
    torch.save(model, f'model6_CNN4_{datasetName}.pt')
    modelScripted = torch.jit.script(model)
    modelScripted.save(f'model6_CNN4_Scripted_{datasetName}.pt')
