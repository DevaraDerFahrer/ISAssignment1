...
def main():
    ...
    model = mylibrary.CNN5(inputChannel, inputSize, numClassess).to(device)
    ...
    for epoch in range(1, numOfEpoch+1):
        ...
        if testAccuracy >= bestAccuracy:
            ...
            torch.save(model, f'model6_CNN5_{datasetName}.pt')
            modelScripted = torch.jit.script(model)
            modelScripted.save(f'model6_CNN5_Scripted_{datasetName}.pt')
