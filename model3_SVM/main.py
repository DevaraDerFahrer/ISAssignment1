...
def ExtractFeatures(device, model, dataLoader):
    ...
    with torch.no_grad():
        for inputs, target in dataLoader:
            ...
            features.append(outputs.cpu()[0])
            labels.append(target.cpu()[0])
...
def main():
    ...
    classifier = GridSearchCV(SVC(), param_grid, cv=5, scoring='accuracy', n_jobs=-1)
    ...
    print(accuracy)
    pickle.dump(bestModel, open(f'model3_SVM1_{datasetName}.pkl', 'wb'))
