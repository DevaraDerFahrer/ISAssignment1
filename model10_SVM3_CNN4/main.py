...
def preprocessMNIST():
    ...
    cnnModel = torch.jit.load(f"../model5_CNN4/model5_CNN4_Scripted_mnist.pt").to(device)
    cnnModel.eval()
...
def preprocessCIFAR10():
    ...
    cnnModel = torch.jit.load(f"../model5_CNN4/model5_CNN4_Scripted_cifar10.pt").to(device)
    cnnModel.eval()
...
def main():
    ...
    if datasetName == "exit":
        ...
        try:
            ...
            validationFeatures = pickle.load(open('validationFeatures_mnist.pkl', 'rb'))
            validationLabels = pickle.load(open('validationLabels_mnist.pkl', 'rb'))
        ...
        try:
            ...
            validationFeatures = pickle.load(open('validationFeatures_cifar10.pkl', 'rb'))
            validationLabels = pickle.load(open('validationLabels_cifar10.pkl', 'rb'))
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    cnnModel = torch.jit.load(f"../model5_CNN4/model5_CNN4_Scripted_{datasetName}.pt").to(device)
    cnnModel.eval()
    packagedModel = mylibrary.SVM3(bestModel)
    pickle.dump(packagedModel, open(f'model10_SVM3_{datasetName}.pkl', 'wb'))
    pickle.dump(bestModel, open(f'model10_SVM3_unpackaged_{datasetName}.pkl', 'wb'))
